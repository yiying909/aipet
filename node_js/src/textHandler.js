...
function promptTerminal(prompt) {
    ...
    // new way so serial port won't have conflict
    return new Promise(resolve => {
        process.stdout.write(prompt)
        process.stdin.setEncoding('utf8')
        process.stdin.once('data', input => {
            resolve(input.trim()) // once get response from terminal, resolve...
        })
    })
}
...
async function askChat(question) {
    ...
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
            ...
            messages: [{ 
                role: 'system', content: "Keep responses under 20 words and provide only the final answer."
            }, {
